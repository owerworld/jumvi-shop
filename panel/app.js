...
function setEditContext(type, id) {
  if (type !== "invoices" && type !== "payments") return;
  editContext[type] = id || null;
}

function clearEditContext(type) {
  if (!type) {
    editContext.invoices = null;
    editContext.payments = null;
    return;
  }
  if (type === "invoices" || type === "payments") editContext[type] = null;
}
window.__jumviDelete = async (type, id, idx) => {
  ...
  if (type === "qcRecords") state.qcRecords = remove(state.qcRecords);
  ...
  if (type === "plannedExpenses") {
    const item = getItem(state.revenueCalc.plannedExpenses || []);
    state.revenueCalc.plannedExpenses = remove(state.revenueCalc.plannedExpenses || []);
    if (item?.paymentId) {
      state.payments = state.payments.filter((p) => String(p.id) !== String(item.paymentId));
    }
  }
  if (type === "invoices" && editContext.invoices === String(id)) clearEditContext("invoices");
  if (type === "payments" && editContext.payments === String(id)) clearEditContext("payments");
  try {
    await saveState();
  } finally {
    renderCurrentTab();
...
window.__jumviEditFromBtn = (btn) => {
  if (!btn) return;
  const type = btn.dataset.type || activeTab;
  const id = btn.dataset.edit;
  if (!id) return;
  setEditContext(type, id);
  renderCurrentTab();
};
...
const i18n = {
  tr: {
    ...
    "kpi.goldSource.truncgil": "Piyasa (TR)",
    "kpi.goldSource.cached": "Önbellek (Son Kayıt)",
  ...
  en: {
    ...
    "kpi.goldSource.cap": "Kapalıçarşı",
    "kpi.goldSource.spot": "Spot (Global)",
    "kpi.goldSource.truncgil": "Market (TR)",
    "kpi.goldSource.cached": "Cache (Last Saved)",
...
async function loadState() {
  ...
  if (candidates.length) {
    ...
    state.revenueCalc = {
      ...defaultState().revenueCalc,
      ...(base.revenueCalc || {})
    };
    if (!Array.isArray(state.revenueCalc.plannedExpenses)) state.revenueCalc.plannedExpenses = [];
  } else {
    state = defaultState();
  }
  syncWiseRefs();
...
function scheduleDelete(type, id, idx) {
  ...
  const getList = () => {
    ...
    if (type === "qcRecords") return state.qcRecords;
    if (type === "experiments") return state.amazonOps.experiments;
    if (type === "launch") return state.amazonOps.launch.items;
    if (type === "plannedExpenses") return state.revenueCalc.plannedExpenses || [];
    return [];
  };
  const list = getList();
  const safeId = (!id || id === "undefined") ? "" : id;
  ...
  if (type === "qcRecords") state.qcRecords = removedList;
  if (type === "experiments") state.amazonOps.experiments = removedList;
  if (type === "launch") state.amazonOps.launch.items = removedList;
  if (type === "plannedExpenses") {
    state.revenueCalc.plannedExpenses = removedList;
    if (item?.paymentId) {
      relatedPayment = state.payments.find((p) => String(p.id) === String(item.paymentId)) || null;
      if (relatedPayment) {
        state.payments = state.payments.filter((p) => String(p.id) !== String(item.paymentId));
      }
    }
  ...
  showUndoToast(t("messages.deleted"), async () => {
    ...
    if (type === "qcRecords") state.qcRecords = restoreList;
    if (type === "experiments") state.amazonOps.experiments = restoreList;
    if (type === "launch") state.amazonOps.launch.items = restoreList;
    if (type === "plannedExpenses") state.revenueCalc.plannedExpenses = restoreList;
    if (type === "plannedExpenses" && pendingDelete.relatedPayment) {
      state.payments.push(pendingDelete.relatedPayment);
    }
    pendingDelete = null;
    await saveState();
    renderCurrentTab();
...
async function refreshGoldPrice() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      showMsg(err?.error ? `Altin kuru guncellenemedi (${err.error})` : "Altin kuru guncellenemedi", "error");
      return null;
    }
    data = await res.json();
  } catch {
    showMsg("Altin kuru guncellenemedi", "error");
    return null;
  }
  const prevQuarterUsd = state.gold.quarterUsd || null;
  const prevUpdatedAt = state.gold.updatedAt || null;
  const prevSource = state.gold.source || null;
  state.gold = {
  ...
  if (data.source === "cached") {
    showMsg("Canli fiyat alinamadi, son kayitli altin kuru gosteriliyor", "error");
    return;
  }
  const changed = Number(prevQuarterUsd || 0) !== Number(state.gold.quarterUsd || 0)
    || String(prevUpdatedAt || "") !== String(state.gold.updatedAt || "")
    || String(prevSource || "") !== String(state.gold.source || "");
  showMsg(changed ? "Altin kuru guncellendi" : "Altin kuru degismedi", "ok");
}
...
function renderStats() {
  ...
  const monthlyInvoiceTotal = state.invoices
    .filter((i) => isInCurrentMonth(i.date))
  ...
  const plannedExpenses = state.revenueCalc?.plannedExpenses || [];
  if (plannedExpenseEditId && !plannedExpenses.some((row) => String(row.id) === String(plannedExpenseEditId))) {
    plannedExpenseEditId = null;
  }
  const editingExpense = plannedExpenseEditId
    ? plannedExpenses.find((row) => String(row.id) === String(plannedExpenseEditId))
    : null;
  const plannedExpensesUsd = plannedExpenses
    .filter((row) => (row?.status || "planned") !== "paid")
    .reduce((sum, row) => sum + (parseMoney(row.amountUsd) || 0), 0);
  ...
  const sourceLabel = state.gold.source === "kapalicarsi"
    ? t("kpi.goldSource.cap")
    : state.gold.source === "spot"
      ? t("kpi.goldSource.spot")
      : state.gold.source === "truncgil"
        ? t("kpi.goldSource.truncgil")
      : state.gold.source === "cached"
        ? t("kpi.goldSource.cached")
        : "";
  ...
  const cards = [
    ...
    },
    {
      key: "gold",
      html: `
    <div class="kpi-card gold span-4 accent" data-kpi="gold">
      <div class="kpi-head">
      ...
      <div class="gold-layout">
        <div>
          ...
          <div class="kpi-mini">Planlanan masraf: ${formatters.currency.format(plannedExpensesUsd)}</div>
          <div class="kpi-mini">Gerçek Net (sadece ödenenler)</div>
          <div class="kpi-value kpi-net">${formatters.currency.format(netBalanceUsd)}</div>
          <div class="kpi-mini">${updatedLabel}</div>
        </div>
        <div class="panel-card gold-scenario-panel">
          <div class="kpi-label gold-scenario-title">Senaryo Masrafları</div>
          <div class="gold-scenario-form">
            <div class="field"><label>Kalem</label><input id="goldScenarioName" placeholder="Kargo / Trademark"></div>
            <div class="field"><label>Tutar (USD)</label><input id="goldScenarioAmount" inputmode="decimal"></div>
            <div class="field"><label>Tarih</label><input id="goldScenarioDate" type="date"></div>
            <button class="btn btn-primary" type="button" id="goldScenarioAdd">${editingExpense ? "Güncelle" : "Ekle"}</button>
            ${editingExpense ? `<button class="btn" type="button" id="goldScenarioCancel">İptal</button>` : ""}
          </div>
          <div class="table-wrap accent-table gold-scenario-table">
            <table>
              <thead><tr><th>Kalem</th><th>Tarih</th><th>Tutar</th><th>Durum</th><th>İşlem</th></tr></thead>
              <tbody>
                ${plannedExpenses.length ? plannedExpenses.map((row, idx) => `
                  <tr>
                    <td>${row.name || "-"}</td>
                    <td>${row.date || "-"}</td>
                    <td>${formatters.currency.format(parseMoney(row.amountUsd) || 0)}</td>
                    <td>${(row.status || "planned") === "paid" ? "Ödendi" : "Planlandı"}</td>
                    <td>
                      ${(row.status || "planned") !== "paid" ? `<button type="button" class="btn btn-xs" data-exp-edit="${row.id || ""}">Düzenle</button>` : ""}
                      ${(row.status || "planned") !== "paid" ? `<button type="button" class="btn btn-xs" data-exp-pay="${row.id || ""}">Ödendi</button>` : ""}
                      <button type="button" class="btn btn-danger btn-xs" data-type="plannedExpenses" data-del="${row.id || ""}" data-del-idx="${idx}" onclick="window.__jumviDeleteFromBtn(this)">Sil</button>
                    </td>
                  </tr>
                `).join("") : `<tr><td class="table-empty" colspan="5">Masraf yok</td></tr>`}
              </tbody>
            </table>
          </div>
        </div>
  ...
  if (statsEl && !statsEl.dataset.deleteBound) {
    statsEl.addEventListener("click", async (e) => {
      ...
      if (delBtn) {
        ...
        if (type === "plannedExpenses" && plannedExpenseEditId === String(id)) plannedExpenseEditId = null;
        scheduleDelete(type, id, idx);
        return;
      }
      const editBtn = e.target.closest("button[data-exp-edit]");
      if (editBtn) {
        plannedExpenseEditId = editBtn.dataset.expEdit || null;
        renderStats();
        return;
      }
      const payBtn = e.target.closest("button[data-exp-pay]");
      if (payBtn) {
        const expenseId = payBtn.dataset.expPay;
        const list = state.revenueCalc?.plannedExpenses || [];
        const row = list.find((x) => String(x.id) === String(expenseId));
        if (!row || (row.status || "planned") === "paid") return;
        const amountUsd = parseMoney(row.amountUsd) || 0;
        ...
        if (plannedExpenseEditId === String(row.id)) plannedExpenseEditId = null;
        await saveState();
        renderStats();
        showMsg("Masraf odendi olarak islendi", "ok");
      }
    });
    statsEl.dataset.deleteBound = "1";
  }
  setupKpiDnD();
  renderAlerts();
  ...
  if (goldRefresh) {
    goldRefresh.onclick = async () => {
      goldRefresh.disabled = true;
      try {
        await refreshGoldPrice();
      } finally {
        goldRefresh.disabled = false;
      }
    };
  }
  const goldScenarioAdd = $("#goldScenarioAdd");
  if (goldScenarioAdd) {
    goldScenarioAdd.onclick = async () => {
      const name = $("#goldScenarioName").value.trim();
      const amountUsd = parseMoney($("#goldScenarioAmount").value);
      const date = $("#goldScenarioDate").value;
      if (!name || !(amountUsd > 0)) return;
      state.revenueCalc.plannedExpenses = state.revenueCalc.plannedExpenses || [];
      if (editingExpense) {
        const row = state.revenueCalc.plannedExpenses.find((x) => String(x.id) === String(editingExpense.id));
        if (row) {
          row.name = name;
  ...
  const goldScenarioCancel = $("#goldScenarioCancel");
  if (goldScenarioCancel) {
    goldScenarioCancel.onclick = () => {
      plannedExpenseEditId = null;
      renderStats();
    };
  }
  if (editingExpense) {
    const nameEl = $("#goldScenarioName");
    const amountEl = $("#goldScenarioAmount");
    const dateEl = $("#goldScenarioDate");
    if (nameEl) nameEl.value = editingExpense.name || "";
    if (amountEl) amountEl.value = formatMoneyInput(editingExpense.amountUsd || 0);
    if (dateEl) dateEl.value = editingExpense.date || "";
  }
...
function renderInvoices(panel) {
  if (editContext.invoices && !state.invoices.find((i) => String(i.id) === String(editContext.invoices))) {
    clearEditContext("invoices");
  }
  const editItem = editContext.invoices
    ? state.invoices.find((i) => String(i.id) === String(editContext.invoices))
    : null;
  const parsed = parseQuery(getSearchTerm());
  ...
  const invoiceSubmitLabel = editItem ? "Faturayı Güncelle" : "Faturayı Kaydet";
  const invoiceCancelBtn = editItem ? `<button class="btn" type="button" id="invoiceCancel">İptal</button>` : "";
  panel.innerHTML = `
    <div class="section">
      <div class="section-header">
      ...
      <div class="panel-grid">
        <div class="panel-side">
          <form class="panel-card" id="invoiceForm">
            ...
            <button class="btn btn-primary" type="submit">${invoiceSubmitLabel}</button>
            ${invoiceCancelBtn}
          </form>
        </div>
        <div class="panel-main">
  ...
  if (editItem) {
    $("#invSupplier").value = editItem.supplierId || $("#invSupplier").value;
    $("#invNo").value = editItem.number || "";
    $("#invDate").value = editItem.date || "";
    $("#invCurrency").value = editItem.amountCurrency || "USD";
    const baseAmount = (editItem.amountOriginal ?? editItem.amountUsd ?? 0);
    $("#invAmount").value = formatMoneyInput(baseAmount);
    if (editItem.amountCurrency === "TRY") {
      $("#invFx").value = editItem.fxRate ? Number(editItem.fxRate).toFixed(4) : "";
      $("#invUsd").value = Number(editItem.amountUsd || 0).toFixed(2);
    } else {
      $("#invFx").value = "";
      $("#invUsd").value = Number(editItem.amountUsd ?? baseAmount ?? 0).toFixed(2);
    }
  }
  ...
  $("#invoiceForm").onsubmit = async (e) => {
    ...
    if (editItem) {
      const idx = state.invoices.findIndex((i) => String(i.id) === String(editItem.id));
      if (idx >= 0) {
        if (fileId && editItem.fileId && editItem.fileId !== fileId) {
          await deleteFile(editItem.fileId, { silent: true });
        }
        state.invoices[idx] = {
          ...editItem,
          ...
          fileId: fileId || editItem.fileId || null
        };
      }
      clearEditContext("invoices");
    } else {
...
function renderPayments(panel) {
  if (editContext.payments && !state.payments.find((p) => String(p.id) === String(editContext.payments))) {
    clearEditContext("payments");
  }
  const editItem = editContext.payments
    ? state.payments.find((p) => String(p.id) === String(editContext.payments))
    : null;
  const parsed = parseQuery(getSearchTerm());
  ...
  const paymentSubmitLabel = editItem ? "Ödemeyi Güncelle" : "Ödemeyi Kaydet";
  const paymentCancelBtn = editItem ? `<button class="btn" type="button" id="paymentCancel">İptal</button>` : "";
  panel.innerHTML = `
    <div class="section">
      <div class="section-header">
      ...
      <div class="panel-grid">
        <div class="panel-side">
          <form class="panel-card" id="paymentForm">
            ...
            <button class="btn btn-primary" type="submit">${paymentSubmitLabel}</button>
            ${paymentCancelBtn}
          </form>
        </div>
        <div class="panel-main">
  ...
  if (editItem) {
    $("#payInvoice").value = editItem.invoiceId || "";
    $("#payDate").value = editItem.date || "";
    $("#payCurrency").value = editItem.amountCurrency || "USD";
    $("#payWise").value = editItem.wiseRef || "";
    const baseAmount = (editItem.amountOriginal ?? editItem.amountUsd ?? 0);
    $("#payAmount").value = formatMoneyInput(baseAmount);
    if (editItem.amountCurrency === "TRY") {
      $("#payFx").value = editItem.fxRate ? Number(editItem.fxRate).toFixed(4) : "";
      $("#payUsd").value = Number(editItem.amountUsd || 0).toFixed(2);
    } else {
      $("#payFx").value = "";
      $("#payUsd").value = Number(editItem.amountUsd ?? baseAmount ?? 0).toFixed(2);
    }
  }
  ...
  $("#paymentForm").onsubmit = async (e) => {
    ...
    if (editItem) {
      const idx = state.payments.findIndex((p) => String(p.id) === String(editItem.id));
      if (idx >= 0) {
        if (fileId && editItem.fileId && editItem.fileId !== fileId) {
          await deleteFile(editItem.fileId, { silent: true });
        }
        ...
        <div class="panel-main">
          ...
          <div class="table-wrap accent-table">
            <table>
              ...
              <tbody>
                ${sorted.length ? sorted.map((q, idx) => `
                  <tr>
                    ...
                    <td><button type="button" class="btn btn-danger" data-type="qcRecords" data-del="${q.id || ""}" data-del-idx="${idx}" onclick="window.__jumviDeleteFromBtn(this)">Sil</button></td>
                  </tr>
                `).join("") : `<tr><td class="table-empty" colspan="6">Sonuç yok</td></tr>`}
              </tbody>
            </table>
          </div>
        </div>
...
function renderProfit(panel) {
  ...
  if (!Array.isArray(calc.plannedExpenses)) calc.plannedExpenses = [];
...
async function importData() {
  ...
  input.onchange = async (e) => {
    ...
    if (file.name.endsWith(".json")) {
      let data;
      try { data = JSON.parse(text); } catch { return showMsg("Geçersiz JSON dosyası", "error"); }
