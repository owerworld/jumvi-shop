...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname.startsWith("/auth")) {
      if (url.pathname.startsWith("/auth/assets/")) {
        const assetRes = await fetch(new Request(PAGES_ORIGIN + url.pathname + url.search, request), {
          cf: { cacheTtl: 0, cacheEverything: false }
        });
        const out = await applySecurityHeaders(assetRes);
        const headers = new Headers(out.headers);
        const forced = contentTypeForPath(url.pathname);
      ...
      const pageMap = {
        "/auth/login": "/auth/pages/login.html",
        "/auth/setup-2fa": "/auth/pages/setup-2fa.html",
        "/auth/forgot-password": "/auth/pages/forgot-password.html"
      };
    ...
    const isPanelAsset = url.pathname.startsWith("/panel/") && /\.(?:js|css|png|jpe?g|svg|webp|ico|woff2?|ttf|map)$/i.test(url.pathname);
    const needsAuth = url.pathname.startsWith("/panel") && !isPanelAsset;
    if (needsAuth) {
    ...
    if (url.pathname.startsWith("/panel/api")) {
      ...
      if (url.pathname === "/panel/api/gold" && request.method === "GET") {
        const getCachedGold = async () => {
          try {
            const obj = await env.PANEL_FILES.get(STATE_KEY);
            if (!obj) return null;
            const text = await obj.text();
            const parsed = JSON.parse(text);
            const gold = parsed?.gold || null;
            if (!gold) return null;
            const quarterTry = Number(gold.quarterTry);
            const quarterUsd = Number(gold.quarterUsd);
            if (!Number.isFinite(quarterTry) || quarterTry <= 0) return null;
            if (!Number.isFinite(quarterUsd) || quarterUsd <= 0) return null;
            return {
              quarterTry: Number(quarterTry.toFixed(2)),
              quarterUsd: Number(quarterUsd.toFixed(2)),
              fxRate: Number((quarterTry / quarterUsd).toFixed(6)),
        ...
        const tryTruncgil = async () => {
          try {
            ...
            const quarterEntry = entries.find(([k]) => {
              const key = String(k || "").toLowerCase();
              return key.includes("ceyrek") || key.includes("çeyrek");
            });
            if (!quarterEntry || typeof quarterEntry[1] !== "object") return null;
            const quarterObj = quarterEntry[1] || {};
            const quarterTry = parseLocaleNumber(
              quarterObj.Satis || quarterObj.Satış || quarterObj.satis || quarterObj.satış || quarterObj.Alış || quarterObj.Alis
            );
            if (!Number.isFinite(quarterTry) || quarterTry <= 0) return null;

            const usdEntry = entries.find(([k]) => String(k || "").toUpperCase() === "USD");
            let tryToUsdRate = NaN;
            if (usdEntry && typeof usdEntry[1] === "object") {
              const usdObj = usdEntry[1] || {};
              const usdTry = parseLocaleNumber(
                usdObj.Satis || usdObj.Satış || usdObj.satis || usdObj.satış || usdObj.Alış || usdObj.Alis
              );
              if (Number.isFinite(usdTry) && usdTry > 0) {
                tryToUsdRate = 1 / usdTry;
              }
            }
            return {
              quarterTry,
              tryToUsdRate: Number.isFinite(tryToUsdRate) && tryToUsdRate > 0 ? tryToUsdRate : null
            };
          } catch {
            return null;
          }
        ...
        const fetchUsdToTryRate = async () => {
          try {
            ...
            if (fxRes.ok) {
              const fxData = await fxRes.json().catch(() => null);
              const rate = Number(fxData?.rates?.TRY);
              if (Number.isFinite(rate) && rate > 0) return rate;
            }
          } catch {}
          try {
        ...
        const tryKapalicarsi = async () => {
          ...
          const item = list.find((row) => row?.code === "CEYREK_YENI")
            || list.find((row) => row?.code === "CEYREK_ESKI")
            || list.find((row) => String(row?.code || "").toUpperCase().includes("CEYREK"))
            || list.find((row) => String(row?.name || "").toLowerCase().includes("ceyrek"));
          if (!item) return null;
          const quarterTry = parseLocaleNumber(item.satis || item.alis || item.sale || item.buy || item.price || "0");
          if (!Number.isFinite(quarterTry) || quarterTry <= 0) return null;
          return { quarterTry };
        };
        ...
        if (cap?.quarterTry) {
          const rate = await fetchTryToUsdRate();
        ...
        const truncgil = await tryTruncgil();
        if (truncgil?.quarterTry) {
          const rate = truncgil.tryToUsdRate || await fetchTryToUsdRate();
          if (Number.isFinite(rate) && rate > 0) {
            const quarterUsd = truncgil.quarterTry * rate;
            return jsonResponse({
              quarterTry: Number(truncgil.quarterTry.toFixed(2)),
              quarterUsd: Number(quarterUsd.toFixed(2)),
              fxRate: rate,
              updatedAt: new Date().toISOString(),
              source: "truncgil"
        ...
        const fetchSpotOunceUsd = async () => {
          ...
          if (key) {
            ...
            if (!metalRes.ok) return null;
            const metalData = await metalRes.json().catch(() => null);
            if (metalData?.status !== "success") return null;
            const ounceUsd = Number(metalData?.metals?.gold);
            if (Number.isFinite(ounceUsd) && ounceUsd > 0) return ounceUsd;
          }
          try {
            const noKeyRes = await fetch("https://api.gold-api.com/price/XAU");
            if (!noKeyRes.ok) return null;
            const noKeyData = await noKeyRes.json().catch(() => null);
            const ounceUsd = Number(noKeyData?.price);
            if (Number.isFinite(ounceUsd) && ounceUsd > 0) return ounceUsd;
          } catch {}
          try {
            const liveRes = await fetch("https://api.metals.live/v1/spot/gold");
            if (liveRes.ok) {
              const liveData = await liveRes.json().catch(() => null);
              const row = Array.isArray(liveData) ? liveData[0] : null;
              const ounceUsd = Array.isArray(row) ? Number(row[1]) : Number(liveData?.price);
              if (Number.isFinite(ounceUsd) && ounceUsd > 0) return ounceUsd;
            }
          } catch {}
          return null;
        ...
        const ounceUsd = await fetchSpotOunceUsd();
        if (!Number.isFinite(ounceUsd) || ounceUsd <= 0) {
          const cached = await getCachedGold();
          if (cached) return jsonResponse(cached);
          return jsonResponse({ error: "gold_failed" }, 502);
        }
        const quarterUsd = (ounceUsd / GOLD_OUNCE_GRAMS) * QUARTER_GOLD_GRAMS * QUARTER_GOLD_PURITY;
        const usdTry = await fetchUsdToTryRate();
        if (!Number.isFinite(usdTry) || usdTry <= 0) {
          const cached = await getCachedGold();
          if (cached) return jsonResponse(cached);
          return jsonResponse({ error: "fx_failed" }, 502);
        }
    ...
    const panelHeaders = new Headers(panelRes.headers);
    panelHeaders.set("Cache-Control", "no-store");
    return applySecurityHeaders(new Response(panelRes.body, {
      status: panelRes.status,
      statusText: panelRes.statusText,
