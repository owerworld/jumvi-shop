...
var GOLD_OUNCE_GRAMS = 31.1035;
var QUARTER_GOLD_GRAMS = 1.75;
var QUARTER_GOLD_PURITY = 22 / 24;
  ...
  async fetch(request, env) {
    ...
    if (url.pathname.startsWith("/panel/api")) {
      ...
      if (url.pathname === "/panel/api/gold" && request.method === "GET") {
        const tryKapalicarsi = /* @__PURE__ */ __name(async () => {
          ...
          if (!item) return null;
          const quarterTry2 = Number(String(item.satis || item.alis || "0").replace(",", "."));
          if (!Number.isFinite(quarterTry2) || quarterTry2 <= 0) return null;
          return { quarterTry: quarterTry2 };
        }, "tryKapalicarsi");
        ...
        if (cap?.quarterTry) {
          ...
          const quarterUsd2 = cap.quarterTry * rate;
          return jsonResponse({
            quarterTry: Number(cap.quarterTry.toFixed(2)),
            quarterUsd: Number(quarterUsd2.toFixed(2)),
            fxRate: rate,
